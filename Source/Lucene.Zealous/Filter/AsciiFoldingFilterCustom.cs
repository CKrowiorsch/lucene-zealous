...
{
    public sealed class ASCIIFoldingFilterCustom : TokenFilter
    {
        readonly ITermAttribute _termAtt;
        char[] _output = new char[512];
        int _outputPos;

        public ASCIIFoldingFilterCustom(TokenStream input) : base(input)
        {
            ...
            if (input.IncrementToken())
            {
                ...
                for (int i = 0; i < length; ++i)
                {
                    ...
                    if (c >= '\u0080')
                    {
                        ...
                        _termAtt.SetTermBuffer(_output, 0, _outputPos);
                        break;
                    }
                }
                return true;
        ...
        void FoldToASCII(char[] buffer, int length)
        {
            // Worst-case length required:
            int maxSizeNeeded = 4 * length;
            if (_output.Length < maxSizeNeeded)
            {
                _output = new char[ArrayUtil.GetNextSize(maxSizeNeeded)];
            }
                ...
                else
                {
                    switch (c)
                    {
                        ...
                        case '\uA73A': // ÃªÅ“Âº  [LATIN CAPITAL LETTER AV WITH HORIZONTAL BAR]
                            _output[_outputPos++] = 'A';
                            _output[_outputPos++] = 'V';
                            break;

                        case '\uA73C': // ÃªÅ“Â¼  [LATIN CAPITAL LETTER AY]
                            _output[_outputPos++] = 'A';
                            _output[_outputPos++] = 'Y';
                            break;
                        ...
                        case '\uFB03': // Ã¯Â¬Æ’  [LATIN SMALL LIGATURE FFI]
                            _output[_outputPos++] = 'f';
                            _output[_outputPos++] = 'f';
                            _output[_outputPos++] = 'i';
                            break;

                        case '\uFB04': // Ã¯Â¬â€ž  [LATIN SMALL LIGATURE FFL]
                            _output[_outputPos++] = 'f';
                            _output[_outputPos++] = 'f';
                            _output[_outputPos++] = 'l';
                            break;
